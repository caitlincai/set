public class Grid {
  ...
  public void removeSet() {
    ...
    for (int i = 0; i < 3; i++) {
      int lastIndex = cardsInPlay - 1 - i;
      Location fromLoc = new Location(col(lastIndex), row(lastIndex));
      Location toLoc = selectedLocs.get(i);
      board[toLoc.getCol()][toLoc.getRow()] = board[fromLoc.getCol()][fromLoc.getRow()];
    }
    cardsInPlay -= 3;
    currentCols--;
  ...
  public void addCardToBoard(Card card) {
    if (cardsInPlay / ROWS < MAX_COLS) {
      int col = cardsInPlay / ROWS;
      int row = cardsInPlay % 3;
      board[col][row] = card;
      cardsInPlay++;
    }
  }
